    ...
    <div className="w-full max-w-6xl mx-auto text-center px-4 sm:px-8">
      ...
      {techCategories.map((category, i) => (
        <div key={i} className="relative w-full mb-10">
          {/* Clipping header */}
          <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-10 px-3 bg-white text-black dark:bg-black dark:text-white text-sm font-semibold whitespace-nowrap">
            {category.title}
          </div>
          ...
          <div className="border-4 border-black dark:border-white/40 rounded-xl pb-6 px-4">
            {/* Grid content */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mt-6 justify-center">
              {category.items.map((tech, idx) => (
                <div
                  key={idx}
                  className="w-full sm:w-40 h-28 flex flex-col items-center justify-center rounded-xl bg-white backdrop-blur-sm dark:bg-white/10 border border-black/20 text-white dark:hover:bg-[#262626] hover:scale-105 hover:shadow-lg hover:shadow-white/20 transition duration-300 ease-in-out"
                >
